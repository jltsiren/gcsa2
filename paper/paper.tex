...
Let $\mathcal{S}$ be a collection of character sequences over alphabet $\Sigma$, and let $k$ be a parameter value. We say that $\mathcal{S}$ is a \emph{\kcollection{k}}, if each sequence $S \in \mathcal{S}$
\begin{enumerate}
...
\end{enumerate}
and the collection contains substrings $\#^{k}$ and $\$^{k}$.
\end{definition}

% FIXME example of strings, DBG, long paths, false positives
% FIXME some comments about DBGs
...
\begin{enumerate}
\item each node $v_{X} \in V$ represent a distinct \kmer{k} occurring in $\mathcal{S}$;
\item $G.\glabel(v_{X}) = X[0]$ and $G.\gkey(v_{X}) = X$ for all $v_{X} \in V$; and
\item each edge $(v_{X}, v_{X'}) \in E$ represents a distinct \kmer{k+1} $Y = Xc = c'X'$ (where $c, c' \in \Sigma$) occurring in $\mathcal{S}$.
\end{enumerate}
...
\begin{proof}
Let $X = S[i, i+\abs{X}-1]$ for a sequence $S \in \mathcal{S}$. For $i \le j \le i+\abs{X}-1$, there is a node $v_{j} \in V$ representing substring $X_{j} = S[j, j+k-1]$, with $G.\glabel(v_{j}) = S[j]$. By definition, the de~Bruijn graph has an edge $(v_{j}, v_{j+1})$ for $i \le j < i+\abs{X}-1$. Hence $P = v_{i} \dotsm v_{i+\abs{X}-1}$ is a path in $G$ and $G.\glabel(P) = X$.
\end{proof}

\begin{lemma}[No false positives with $\abs{P} \le k+1$]\label{lemma:dbg-fp}
Let $G = (V, E)$ be an \orderk{k} de~Bruijn graph of \kcollection{k} $\mathcal{S}$, and let $P$ be a path in $G$ with $\abs{P} \le k+1$. Then the label of the path is a substring of a sequence $S \in \mathcal{S}$.
\end{lemma}

\begin{proof}
If $\abs{P} = 0$, the label is an empty string and hence a substring of each sequence $S \in \mathcal{S}$.
If $\abs{P} = 1$, the label is equal to $G.\glabel(v_{0})$, which is a prefix of $G.\gkey(v_{0})$ and hence a substring of $\mathcal{S}$.
Now assume that $G.\glabel(P) = cX$, where $c \in \Sigma$ and $1 \le \abs{X} \le k$. By induction, string $X$ is a prefix of $G.\gkey(v_{1})$. As edge $(v_{0}, v_{1})$ exists, $c \cdot G.\gkey(v_{1})$ must be a substring of $\mathcal{S}$. Hence $cX$ is a substring of $\mathcal{S}$, and also a prefix of $G.\gkey(v_{0})$ if $\abs{cX} \le k$.
\end{proof}
...
\begin{definition}[Maximal paths]
...
We can build a \kcollection{k} of maximal path labels by starting with the collection $\mathcal{S}$ of maximal path labels. In order to transform $\mathcal{S}$ into a \kcollection{k}, we adjust the number of characters $\#$ at the beginning of each non-left-infinite sequence, and the number of characters $\$$ at the end of each non-right-infinite sequence. We also add string $\#^{k} \$^{k}$ into the collection if necessary.

\begin{definition}[de~Bruijn graph of a graph]
Let $G = (V, E)$ be a labeled graph, and let $\mathcal{S}$ be a \kcollection{k} of maximal path labels in $G$. The \orderk{k} de~Bruijn graph of $\mathcal{S}$ is also the \orderk{k} de~Bruijn graph of graph $G$.
\end{definition}
...
\begin{enumerate}
\item each node $v_{K} \in V'$ represent a distinct key $K \in \mathcal{K}$;
\item $G'.\glabel(v_{K}) = K[0]$ and $G'.\gkey(v_{K}) = K$ for all $v_{K} \in V$; and
...
using pruned de Bruijn graphs as indexes; false positives
