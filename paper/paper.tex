...
% hyperref does not work with LNCS
%\usepackage{hyperref}
\usepackage{url}
...
Suffix trees, suffix arrays, and the \FMindex{} can be generalized to index multiple texts. There are also generalizations for other combinatorial structures. The \emph{XBW transform} \cite{Ferragina2009b} is essentially an \FMindex{} for \emph{labeled trees}. The nodes of the tree are sorted by path labels from the node to the root. $\BWT$ stores the labels of the children of each node (leaf nodes require special treatment). If a node has $k$ children, we encode that as a binary sequence $0^{k-1} 1$. We concatenate these sequences to form the indegree bitvector $\bvIN$. The labels of the children of the $i$th node are found in $\BWT[\bvIN.\select(i, 1) + 1, \bvIN.\select(i + 1, 1)]$.

The \emph{generalized compressed suffix array} (GCSA) \cite{Siren2014} extends the XBW transform to a class of graphs that includes \emph{directed acyclic graphs} and de Bruijn graphs. Before indexing, we transform the graph into an equivalent graph, where the nodes can be unambiguously sorted by the labels of the right-maximal paths starting from them. The transformation increases the size of the graph exponentially in the worst case. In addition to sequences $\BWT$ and $\bvIN$, GCSA also uses the outdegree bitvector $\bvOUT$, which is encoded in the same way as $\bvIN$. \LFmapping{} uses $\select$ queries on bitvector $\bvIN$ to map nodes to BWT ranges, ordinary \LFmapping{} with $\BWT$ to map incoming edges to the corresponding outgoing edges, and $\rank$ queries on bitvector $\bvOUT$ to map the outgoing edges to the predecessor nodes.
...
The proofs of the lemmas in this section can be found in Appendix~\ref{appendix:proofs}.

\subsection{De Bruijn Graphs as Path Indexes}

The \emph{\kmer{k} index} is the simplest path index. It consists of a set of \emph{key-value pairs} $(X, V_{X})$, where $X \in \Sigma^{k}$ is a \kmer{k} and $V_{X} \subseteq V$ is the set of start nodes of the paths matching the \kmer{k}. If we store the pairs in a hash table, we can quickly search for patterns of length $k$. If we store the pairs as a sorted list, queries become slower, as they require binary search. On the other hand, we gain the ability to search for patterns shorter than $k$ characters. The main drawback of these basic \kmer{k} indexes is their size, as they have to store the key-value pairs explicitly.
...
Let $G' = (V', E')$ be a de~Bruijn graph of graph $G = (V, E)$, and let $\mathcal{S}$ be the \kcollection{k} used to define it. We attach a set of nodes of graph $G$ to each node $v' \in V'$ as a \emph{value}: $G'.\gvalue(v) = \set{ \mathcal{S}.\gnode(S, i) \mid S \in \mathcal{S}, S[i, i+k-1] = G'.\gkey(v)}.$ Apart from some technicalities near the source/sink nodes, set $G'.\gvalue(v)$ is the set of start nodes of the paths $P$ matching pattern $G'.\gkey(v)$ in graph $G$. There will be no \emph{false negatives} (path labels that exist in the input graph but not in the index), regardless of pattern length. There may be \emph{false positives} (path labels that exist in the index but not in the input graph) with patterns longer than $k$, but we can avoid them by \emph{verifying} the results in the input graph. See Figure~\ref{figure:graph-dbg} for an example.
...
We can encode the de~Bruijn graph as a GCSA using $(\log \sigma + 2)(1 + \oh{1})$ bits per \kmer{k}. By using a similar sampling scheme for the values as in the FM-index, we get a \kmer{k} index that typically uses a couple of bytes per \kmer{k} (see the \orderk{32} index in Section~\ref{sect:experiments}). While this is much less than with the basic \kmer{k} indexes, it is still too much for large variation graphs.
...
\caption{Left: An \protect\orderk{3} pruned de~Bruijn graph $G''$ \protect\kequivalent{3} to the de~Bruijn graph in Figure~\ref{figure:graph-dbg}. Right: GCSA for graph $G''$. Leftward arrows show how backward searching works, with the red arrows showing it from pattern $\mathtt{T}$ to pattern $\mathtt{AT}$. Rightward arrows match nodes with sampled values, with the blue ones showing them for node $\mathtt{CAT}$.}\label{figure:pruned-index}
\end{figure}
...
GCSA2 is the path indexing library of vg \cite{Garrison2014-2016}. The implementation is written in C++, and the source code is available on GitHub (\url{https://github.com/jltsiren/gcsa2}). Most index components are encoded using fast non-compressed bitvectors. As GCSA2 uses \emph{SDSL} \cite{Gog2014b} and \emph{libstdc++ parallel mode}, it requires g++ version~4.9 or later to compile.

We used a system with two 16\nobreakdash-core AMD Opteron 6378 processors and 256~gigabytes of memory for the experiments, and stored all files on a distributed Lustre file system. The system was running Ubuntu~12.04 on Linux kernel~3.2.0. We used vg version~1.3.0 for processing the graphs and GCSA2 version~0.7 for the benchmarks.
...
\begin{table}[t!]
\begin{center}
\caption{GCSA2 index sizes. Order $k$ of the path graph; number of \kmer{k}s and nodes in the path graph in billions; index size in gigabytes and in bits per \kmer{k} for the graph ($\BWT$, $\bvIN$, and $\bvOUT$), the index (graph, $B_{S}$, $B_{V}$, and $V_{S}$), and the index with the extensions from Appendix~\ref{appendix:extensions}.}\label{table:indexes}
...
With \kmer{16}s, where both indexes match a similar number of patterns, $\find$ queries in GCSA2 are about 3x slower than in the FM\nobreakdash-index. With longer patterns, the difference grows larger, as the query stops early when the index cannot match the pattern. $\select$ queries on bitvector $\bvIN$ are the main source of the difference, as they are much slower than $\rank$ queries. A simplified GCSA encoding (see Appendix~\ref{appendix:encoding}) would make $\find$ queries faster and $\locate$ queries slower.

When the number of distinct occurrences is high, $\locate$ queries are about 5x slower than in the FM\nobreakdash-index. With less occurrences, unpredictable factors such as SA order sampling in the FM\nobreakdash-index or nodes with multiple values and filtering out duplicates in GCSA2 affect the results. Queries $\parent$ and $\countq$ are both clearly faster than a single step of backward searching.
...
We can build a whole-genome index overnight on a system with 96~gigabytes of memory and a few hundred gigabytes of fast disk space. The resulting index takes less than 16~gigabytes, or 1.12~bits per \kmer{k} for the \orderk{128} index with extensions. Queries are several times slower than in a similar implementation of the FM\nobreakdash-index for sequences.

The primary desing goals in GCSA2 were query performance and index size. The index works with arbitrary graphs, supports queries that are long enough to map short reads in one piece, and provides several options for dealing with complex regions. Other path indexes work with a more restricted class of graphs \cite{Siren2014,Huang2013,Kim2015-2016,Maciuca2016}, are at least an order of magnitude slower \cite{Huang2013,Maciuca2016}, require much more space \cite{Bowe2012,Pell2012,Cazaux2014,Marcus2014}, or are theoretical proposals that have never been implemented \cite{Thachuk2013}.

We may want to determine whether a pattern matches known \emph{haplotypes} or only their recombinations. As GCSA2 does not support this directly, vg checks it afterwards using a separate structure \cite{Novak2016}. The \emph{FM\nobreakdash-index of alignment} \cite{Na2015,Na2016} embeds the haplotypes directly in a GCSA-like index and reports the haplotypes matching the $\find$ query. While the solution depends on the specific properties of the graph, it could be possible to extend it to work with any GCSA.
...
Furthermore, $v_{0} = \mathcal{S}.\gnode(S, i) \in G'.\gvalue(v'_{i}) \subseteq G'.\locate(X)$. \qed
\end{proof}
...
Set $M_{i}$ is the set of all nodes $u' \in V'$ such that substring $X[i, i+m]$ is a prefix of key $G'.\gkey(u')$. If $\abs{M_{i}} > 1$, string $X[i, i+m]$ is a proper prefix of key $G'.\gkey(u')$ for all nodes $u' \in M_{i}$ due to the prefix-free property, and we can continue with context length $m+1$. Otherwise the context length becomes $\min(m+1, \abs{G'.\gkey(u')})$ for the only node $u' \in M_{i}$. \qed
\end{proof}
...
Given $v_{0} \in G_{d}.\find(X)$, we can build a path $P'' = v''_{0} \dotsm v''_{\abs{X}-1}$ matching the pattern in graph $G''$, with $v_{0} \in V_{d}(v''_{0})$. We can also start from path $P''$ and determine the node $v_{0}$. As $G_{d}.\gvalue(v_{0}) = G''.\gvalue(v''_{0})$, we have $G_{d}.\locate(X) = G''.\locate(X)$. \qed
\end{proof}
...
The above is true for all nodes $x' \in G'.\gpred(v', G'.\glabel(u'))$. Hence we can prune graph $G'$ further using string $G'.\glabel(u') \cdot G'.\gkey(v')$ as the new key in Lemma~\ref{lemma:dbg-prune}. \qed
\end{proof}
