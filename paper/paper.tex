...
\begin{lemma}[Consequences of maximal pruning]\label{lemma:dbg-maximal}
...
\end{itemize}
If node $v \in V$ has lexicographic rank $i$, the range of incoming edges $(u, v) \in E$ to the node is $[sp_{in}, ep_{in}] = [\bvIN.\select(i, 1) + 1, \bvIN.\select(i+1, 1)]$, with $sp_{in} = 0$ if $i = 0$. The labels of predecessor nodes $u$ are encoded in $\BWT[sp_{in}, ep_{in}]$. Sorting the incoming edges by pairs $(\BWT[j], i)$, where $\BWT[j]$ corresponds to edge $(u, v)$, is equivalent to sorting them by strings $G.\glabel(u) \cdot G.\gkey(v)$. As multiple edges may have the same sort keys, our sorting algorithm must be stable. We can achieve the desired sorting order by using \LFmapping: $j \mapsto \LF(j)$.

Consider now the outgoing edges. If the lexicographic rank of node $u \in V$ is $i'$, the range of outgoing edges $(u, v) \in E$ is $[sp_{out}, ep_{out}] = [\bvOUT.\select(i', 1) + 1, \bvOUT.\select(i'+1, 1)]$, with $sp_{out} = 0$ if $i' = 0$. The edges are already sorted by keys $G.\gkey(u)$. All outgoing edges from the same node have the same sort key, as there is nothing to distinguish them from one another. Because graph $G$ is a path graph, we know that key $G.\gkey(u)$ prefix-matches string $G.\glabel(u) \cdot G.\gkey(v)$. The sorting orders are therefore compatible. For every $j \in [sp_{in}, ep_{in}]$ for node $v \in V$, having $\LF(j) \in [sp_{out}, ep_{out}]$ for node $u \in V$ implies an edge $(u, v) \in E$. If we sort the incoming edges $(u, v) \in E$ stably by strings $G.\glabel(u) \cdot G.\gkey(v)$, we have $(u, v) < (u', v')$ in that order, if $G.\gkey(u) < G.\gkey(u')$.

We implement query $G.\find(X)$ with backward searching. Let $X \in \patternset$ be a pattern. If $\abs{X} = 0$, query $G.\find(X)$ returns the lexicographic range $[0, \abs{V}-1]$ containing all nodes of the path graph. Now assume that $\abs{X} \ge 1$ and that query $G.\find(X[i+1, \abs{X}-1])$ has returned range $[sp_{i+1}, ep_{i+1}]$. We want to find the lexicographic range corresponding to the union of sets $G.\gpred(v, X[i])$ over nodes $v \in G.\find(X[i+1, \abs{X}-1])$. We start by mapping the node range $[sp_{i+1}, ep_{i+1}]$ into range
$$
[sp_{in}, ep_{in}] = [\bvIN.\select(sp_{i+1}, 1) + 1, \bvIN.\select(ep_{i+1}+1, 1)]
$$
of incoming edges (with $sp_{in} = 0$ if $sp_{i+1} = 0$). Then we map that range into the corresponding range of outgoing edges
$$
[sp_{out}, ep_{out}] = [\LF(sp_{in}, X[i]), \LF(ep_{in}+1, out) - 1].
$$
The lexicographic range containing nodes $G.\find(X[i, \abs{X}-1])$ is
$$
...
Two encodings: general and the one for $\abs{G.\gpred(v, c)} \le 1$. Difference: no general LF in the latter one, must go forward for samples.
